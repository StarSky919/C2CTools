...
export class Dialog {
  constructor(options) {
    ...
    this.button('确定');
    this._buttons.children[0].classList.add('full-width');
  }
  ...
  button(text, func = noop) {
    if (!isEmpty(this._buttons.children)) {
      ...
      this._buttons.children[0].classList.remove('full-width');
    }
  ...
  getButton(index) {
    ...
    return {
      ...
      onClick(func = noop) {
        bindOnClick(button, async event => await func.call(_this, close) !== false && await close());
      }
    };
  }
...
export class ItemSelectorDialog extends Dialog {
  constructor(options) {
    super(options);
    const { storage, settingName, defaultValue } = options;
    ...
    if (!isNullish(settingName) && !isNullish(defaultValue) && !this.storage.has(settingName)) this.storage.set(settingName, defaultValue);
  }
  ...
  show() {
    ...
    for (const { id, text } of this.items) {
      ...
      const saved = this.storage.get(settingName);
    ...
    this.button('确定', close => {
      ...
      if (!isNullish(settingName)) this.storage.set(settingName, id);
